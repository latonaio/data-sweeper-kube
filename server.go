...
func deleteFiles(dirPath string, excludeFiles map[string]struct{}, isRecursive bool) error {
	...
	for _, file := range files {
		...
		if file.IsDir() {
			if isRecursive {
				deleteFiles(filepath.Join(dirPath, file.Name()), excludeFiles, isRecursive)
			}
		} else if _, ignore := excludeFiles[file.Name()]; ignore == true {
...
func sweep(c echo.Context) error {
	...
	deleteFiles(m.DirPath, generateSet(m.ExcludeFiles), m.IsRecursive)
